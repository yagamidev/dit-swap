...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ditcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ditcoin
    // Mac: ~/Library/Application Support/Ditcoin
    // Unix: ~/.ditcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ditcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ditcoin.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;

    // Load old config file if present
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        boost::filesystem::path pathOldConfigFile = GetDataDir(false) / "ditcoin.conf";
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ditcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Ditcoin will not work properly.");
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Ditcoin:" << FormatVersion(DITCOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
