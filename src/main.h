...
public:
    ...
    {
        // ditcoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // ditcoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // ditcoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // ditcoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // ditcoin: whether transaction is a coinstake
    bool fCoinStake;

    // ditcoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // ditcoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // ditcoin
            undo.nTime = nTime;            // ditcoin
        }
        return true;
    }
...
public:
    ...
    // ditcoin: block signature - signed by coin base txout[0]'s owner
    ...
    // ditcoin: two types of block: proof-of-work or proof-of-stake
    ...
    // ditcoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // ditcoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // ditcoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (ditcoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // ditcoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // ditcoin: proof-of-stake related block index fields
    unsigned int nFlags;  // ditcoin: block index flags
    enum
    {
